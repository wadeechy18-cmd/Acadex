...
def list_plans(db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> list[LessonPlanSummaryResponse]:
    return [_summary_response(db, p) for p in lesson_plan_service.list_plans(db, user)]
